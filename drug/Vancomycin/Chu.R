...
des_intro <- "Vancomycin iv model for Chinese patients with augmented renal clearance(≥130ml/min)"
...
mod_lcov_value <- NULL
mod_cov_abbr <- c("Age(Data were collected from patients aged 11–82 years)")
...
sd_eta <- sqrt(c(0.1588, 0.5403)) # put sd^2 value in this vector

# Model file for estimation -----------------------------------------
f <- function() {
  ini({
    theta1 <- c(log(8.52))     # CL
    theta2 <- c(log(155.4))    # V
    theta3 <- c(0.2409)        # Proportional error
    eta1 ~ c(0.1588)           # Cl
    eta2 ~ c(0.5403)           # V
  })
  model({
    cl <- (1-(0.012*(AGE-45))) * exp(theta1 + eta1)
    v <- exp(theta2 + eta2) 
    ke = cl/v
